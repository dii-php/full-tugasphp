...
<title>Form Nilai Siswa - Kelulusan</title>
<style>
...
<body>
<header>
    <h1>Form Kelulusan Siswa</h1>
    <p>Oleh: Moh. Novrialdi Pratama</p>
</header>
<div class="container">

<?php
$koneksi = new mysqli("sql102.infinityfree.com", "if0_39908296", "pantoloan87", "if0_39908296_kelulusan");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($bindo < 0 || $bing < 0 || $mtk < 0 || $kejuruan < 0) {
        ...
        // Cek duplikat nama
        $cek = $koneksi->prepare("SELECT * FROM siswa WHERE LOWER(TRIM(nama)) = ?");
        ...
        if ($hasil->num_rows > 0) {
            ...
            $lulus = ($rata >= 75) ? "Lulus" : "Tidak Lulus";

            $stmt = $koneksi->prepare("INSERT INTO siswa (nama, bindo, bing, mtk, kejuruan, rata_rata, predikat, lulus)
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("siiiidss", $nama, $bindo, $bing, $mtk, $kejuruan, $rata, $predikat, $lulus);
...
<!-- FORM INPUT -->
<form action="kelulusan.php" method="post" novalidate>
    ...
    <input type="number" name="bindo" id="bindo" min="0" max="99" required oninput="if(this.value>99)this.value=99;">

    <label for="bing">Nilai Bahasa Inggris:</label>
    <input type="number" name="bing" id="bing" min="0" max="99" required oninput="if(this.value>99)this.value=99;">

    <label for="mtk">Nilai Matematika:</label>
    <input type="number" name="mtk" id="mtk" min="0" max="99" required oninput="if(this.value>99)this.value=99;">

    <label for="kejuruan">Nilai Kejuruan:</label>
    <input type="number" name="kejuruan" id="kejuruan" min="0" max="99" required oninput="if(this.value>99)this.value=99;">
...
<div class="table-container">
    ...
    <table>
        ...
        <?php
        $result = $koneksi->query("SELECT * FROM siswa ORDER BY id ASC");
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>{$row['rata_rata']}</td>
                        <td>{$row['predikat']}</td>
                        <td><b style='color:" . ($row['lulus']=='Lulus'?'green':'red') . ";'>{$row['lulus']}</b></td>
                      </tr>";
                $no++;
            }
        } else {
            echo "<tr><td colspan='9'>Belum ada data nilai</td></tr>";
        }
        ?>
    </table>
</div>
